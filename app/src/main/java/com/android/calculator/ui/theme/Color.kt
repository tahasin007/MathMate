package com.android.calculator.ui.theme

import androidx.compose.ui.graphics.Color

val PrimaryLight = Color(0xFFFFFFFF)
val OnPrimaryLight = Color(0xFF000000)
val OnSecondaryLight = Color(0xFFB80C0C)
val TertiaryLight = Color(0xFFFFF5F5)
val OnTertiaryLight = Color(0xFFF84141)

val PrimaryDark = Color(0xFF292d31)
val OnPrimaryDark = Color(0xBEC2CFCE)
val OnSecondaryDark = Color(0xBE1A8F85)
val TertiaryDark = Color(0xFF2C2F31)
val OnTertiaryDark = Color(0xBE44FFEF)

val RippleColorLight = Color(0xFFDBDBDB)
val RippleColorDark = Color(0xFF2B2B2B)